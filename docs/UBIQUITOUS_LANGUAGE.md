...
| **DIA / FL / LOC / OAL / T# / Ø / `*`** (printed abbreviations) | The ratified spec-label vocabulary (2026-08-15): **DIA**/**Ø** = diameter, **FL** = flute count (`geometry.flutes`), **LOC** = length of cut = `geometry.cutting_edge_height`, **OAL** = overall length = `geometry.length` (this pins the schema semantics: canonical `length` MEANS overall length), **T#** = the bound entry's `tool_number`, and **`*`** after a value = measured differs from catalog nominal (e.g. a reground `Ø5.92*` on a nominal 6mm tool). Dimensions print **decimal as stored** with `(in)`/`(mm)` where space allows — no unit conversion, no fractional-inch display (deferred as pure presentation). |

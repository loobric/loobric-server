...
| **Machine** | First-class entity: a CNC machine — identity, controller type, definition (axes/spindle/limits/post), and canonical **capability** sections: `spindle` (`max_rpm`/`min_rpm`/`power`/`taper` — declared limits, not runtime telemetry) and `coolant` (`flood`/`mist`/`through_spindle` — what the machine has, not what is running). Capability answers "what can this machine do?" from the record itself, never via tool-list archaeology or a client's opaque definition blob. Asserted only (no observe door); vocabulary is ratified, not accreted (unknown capability path → 400). Clients sync their native machine definitions to it. Public path `/api/v1/machine-records`. |
| **ToolSet** | A control/CAM-agnostic named collection of tools — purely CAM-owned — with ordered, provenance-tagged member numbers. A member's `number` is the CAM side's durable **claim** (asserted, or unknown): the T-number posted G-code will call. Observation never overwrites a claim; when a machine runs the set as its active **Setup**, reads carry the entry's `observed` number and a derived `state` alongside the untouched claim. A FreeCAD `.fctl`, a Fusion library, or a shop drawer is one client's representation of a ToolSet, living in `clients.<name>.data`. (Supersedes **Library**, purged 2026-06-11; the old set-level `machine_id` link was replaced by Setups in 0.7.0.) |
| **Setup** | The period during which a ToolSet is active on a Machine — the transitory machine↔set relationship (MAPPING_PLAN.md, grilled & locked 2026-07-29). An operator act: `loobric use-set <machine> <set>` activates (atomically ending the prior setup, which survives as history); lifecycle rides the **bind** door, so agent keys and bare controller sync keys can never switch setups. Activating changes NOTHING on either side — it only changes what the machine's status view compares against. Internal entity `machine_set_maps` (one `active` row per machine, DB-enforced); the noun users meet is "setup", never the table name. |
| **ready** | The headline of a machine's setup view (`loobric status`): every claim of the active set is **satisfied** — mounted, at the claimed number, with confirmed identity. Exactly that scope: it says nothing about offsets sanity or tool life. Notes never count against readiness. The server is **never an interlock** — `ready` informs; individual clients may enforce locally. |
| **attention / notes** (severity) | The two derived severity classes of a setup view, computed from direction, never stored: lines where the list's claim is unmet or a shared row disagrees **need attention** (`requested` / `mismounted` / `blocked` / `pending bind`); table rows the list doesn't claim are **notes** (`unlisted` / `unknown tool`) — real differences, informational only, never alarms, never blocking. |
...
| **satisfied** (claim state) | A member of the active set whose instance is bound to an entry at the claimed number (or with no claim yet): mounted, right number, identity confirmed. Renders as **ok**. One of the derived claim states (`satisfied` / `requested` / `mismounted` / `blocked` / `pending bind`), computed at read time, never stored. (Replaces the retired `loaded`.) |
| **requested** (claim state) | A member of the active set that nothing on the machine holds — a load request awaiting the operator, not stale data. Its `number` is the **asserted** claim the client supplied, or **unknown**. Shows up as "requested — not on machine". (More members than machine entries is a valid state when the surplus are `requested` / `pending bind`.) |
| **mismounted** (claim state) | The member's tool IS mounted and confirmed, but at a different number than claimed (CAM says T14, table has T9). Both numbers are shown; resolution is the operator remounting or the programmer renumbering the claim and reposting — never a silent renumber. |
| **blocked** (claim state) | The claimed number is held by a DIFFERENT confirmed tool — the dangerous case: posted G-code calling that number would run the wrong tool. The occupant is named. Deliberately has no dismiss/ignore: the only honest resolutions change the world or the claim. |
| **pending bind** (claim state) | Something is mounted for the claim — an entry at the claimed number, or an open proposal names the instance — but identity is unconfirmed. Indistinguishable from `blocked` until someone binds, so it is never guessed into a flavor. Flips to `satisfied` on confirm. |
| **unlisted / unknown tool** (note states) | Table rows the active set doesn't claim: `unlisted` = bound to a tool outside the set (the probe, the last job's leftovers); `unknown tool` = an unbound entry (something's there, identity unknown — bind or create-record settles it). Notes, per the severity rule above. |
| **Usage** | Cross-machine tool life (TOOL_SCHEMA.md §7.8; implemented 2026-08-04). An entry's `usage_hours` is the controller's own resettable counter, `observed` verbatim — that machine's ledger, nothing more. An instance's lifetime total (`usage.hours`) is **`derived:usage-ledger`** — never observed, never asserted, no door writes it (400) — and decomposes into per-machine contributions; that decomposition IS the provenance. Counters are ingested as **deltas, never gauges**; the first observation is a baseline and credits nothing. The derived total is the one usage fact the **public spec page** shows. |
| **Usage ledger / contribution** | The internal append-only entity (`usage_ledger`) behind derived usage: each row is a **contribution** — a positive counter delta credited to the entry's confirmed bound instance (binding unchanged across the interval), attributed `observed:<client>@<machine>`. Resets re-baseline; ambiguous hours are **orphaned** (`instance_id` null, surfaced read-only in the Inbox as `usage_orphans`, never guessed). Like `machine_set_maps`, the noun users meet is "usage", never the table name. |
| **Label** | A physical QR/short-code sticker pointing at one record — the drawer↔digital link. Noun AND verb (founder-chosen, 2026-08-04): you **label** a record / **unlabel** it; a label not on anything is **blank**. Short Crockford-base32 code, printed human-readable beside the QR, resolving at `<base>/t/<code>` (outside `/api` — permanent ink outlives API versions; base URL is per-instance, self-hosting first-class). Entity-generic `(entity_type, entity_id)`, many labels may point at one record; v1 exposes tool-instance labels. Owner-private rows (`/api/v1/labels`); **only the generating account can label with a code**. Deliberately NOT "bind" — bind stays entry↔instance only. Labeling rides the **bind door** (same physical↔digital adjudication); creating labels rides assert. Lifecycle: **unlabel** = deliberate reuse (label reverts to blank); **deleting a record burns its labels** (codes dead forever — delete is for records that should never have existed; a *retired* tool is a future `status` value and keeps its labels). |
| **Public spec page** | What an anonymous scan of a labeled tool sees: the record's catalog identity and geometry, plus derived usage totals once they exist. **Labeling a record is what makes this page exist** — unlabeled records are never publicly reachable. Governed by the *physical-inspection rule*: the page reveals what inspecting the artifact would reveal (what it is, its accumulated condition), never attribution — no owner, no machines, no event history. **Publish the sum, never the ledger**: provenance sources are reduced to their kind (`observed`/`asserted`/`derived`); `derived:usage-ledger` is the one full source shown. Built by allowlist construction in `public_view.py`; the no-owner-leak invariant is a tested security assumption (#18). |
| **Spec label** | A printed *rendering of a record* — specs in ink for at-the-toolbox identification (grilled & ratified 2026-08-15). **Not a Label row**: a Label is identity (code → record); a spec label is a report of the record, on which a Label's QR/code is one *optional* element (a QR-less spec plaque is a spec label with no code, not a contradiction). Early-bound by nature: everything printed is a **snapshot at print time** — reprint after regrind/rebind; the QR, when present, stays late-bound and always resolves to current truth. Values are the **merged view** (instance measured wins over catalog nominal), measured-differs-from-nominal is **marked** (`*`); T# prints from the **bound entry only** (unbound → omitted; set claims never print); the tool image is a **derived silhouette** computed from geometry, never stored media. v1: canned fixed-content templates on canned stocks + a JSON/CSV export escape hatch for bring-your-own-layout printing. `POST /api/v1/spec-labels/sheet`, **read door** — printing never mints; unlabeled records under a QR template are a 400 naming the ids. See `docs/SPEC_LABELS.md`. |
| **DIA / FL / LOC / OAL / ANG / tip / T# / Ø / `*`** (printed abbreviations) | The ratified spec-label vocabulary (2026-08-15): **DIA**/**Ø** = diameter, **FL** = flute count (`geometry.flutes`), **LOC** = length of cut = `geometry.cutting_edge_height`, **OAL** = overall length = `geometry.length` (this pins the schema semantics: canonical `length` MEANS overall length), **ANG** = included angle = `geometry.included_angle`, printed with `°`, **tip** = tip diameter = `geometry.tip_diameter`, **T#** = the bound entry's `tool_number`, and **`*`** after a value = measured differs from catalog nominal (e.g. a reground `Ø5.92*` on a nominal 6mm tool). A pointed tool spec'd by angle rather than diameter (engraver, chamfer — cutting diameter is a function of depth) prints angle-first: `60° · tip 0.005` replaces the `Ø` lead. Dimensions print **decimal as stored** with `(in)`/`(mm)` where space allows — no unit conversion, no fractional-inch display (deferred as pure presentation); the angle's `deg` never becomes the line's unit suffix. |
| **Cutting data preset** ("preset") | An F&S **recommendation with a source, never a fact about the tool** (grilled 2026-08-16; the word became available when R6 evicted the v1 setup-data squatter). Canonical `presets` is a **derived, normalized union** (`derived:preset-union`) of source-preserved contributions — the server never reconciles and never picks a winner; identical values from two sources are corroboration, kept as two entries. Entry identity is **(origin, label)**; same-origin re-contribution replaces its predecessor (replace-own), humans get full CRUD, agents never delete. Normal form `preset_schema: 1` = the G5 engineering values (Vc, Fz, vertical-feed ratio, material `{uuid?, name}` verbatim, ratified `op_type`) **plus a verbatim extras bag** (non-comparable by design); raw feed/RPM are never persisted. Floor: material + ≥1 engineering value joins the union, wholly-untranslatable presets stay client-section-only and are counted, sync never fails over presets. Entries live on catalog records (manufacturer recommendations) AND instance records (shop-proven, optionally machine-qualified); an instance's preset listing unions both, origin-marked. Intake: the audited contribution door (`POST …/{id}/presets`) — clients promote their own translations (the `.fctl` number-promotion pattern; the server never parses client sections), importers and agents use the same door. The **origin** is the recommender (`manufacturer`, `freecad`, `user`, an agent); the provenance actor is the transcriber — same split as the manufacturer-QA door. |
| **op_type** (preset vocabulary) | The ratified operation taxonomy for cutting data presets — ratified, not accreted, like status: the contribution door 400s on unknown values; sync-side mapping preserves the source's verbatim op string in the entry's extras, and an unmappable op_type is honestly absent. Seeded 2026-08-16: `profiling`, `slotting`, `pocketing`, `adaptive`, `facing`, `drilling`, `boring`, `threading`, `engraving`, `chamfering`. |
| **retired** (status) | The first ratified instance `status` value (2026-08-04): a *valid record* of a tool no longer in service. Assert-only (a judgment, not a measurement — no observable status values exist); absence of status = in service. A retired tool keeps its labels, public spec page, and usage history; retirement is **owner-only** (never shown on the public page). Binding a retired tool is allowed but surfaced ("retired tool mounted"), never blocked. Distinct from **deletion** (data management for records that should never have existed; burns labels). Status values are ratified, not accreted — anything unratified is a 400. |
...
| **Catalog (vs ToolCatalogRecord)** ⚠️ | "Catalog" means a **named collection of catalog records** — first-class since 2026-08-16 (grilled: the one-giant-pile problem): a sectioned record (`canonical.name` + `canonical.members`, a provenance-tagged list of catalog-record ids) at `/api/v1/catalogs`. Membership is **many-to-many organization, never identity**: a record may sit in any number of catalogs ("Harvey 2026" from an import AND a curated "engraving cutters"), uncataloged records are allowed (the UI surfaces them as *Uncataloged*, visible and drainable), and the account-wide natural key (manufacturer, product_code) is untouched by grouping. Importers auto-create/reuse a catalog named from the source (single manufacturer, else the filename), so imports stop feeding the pile. Deleting a catalog never deletes its records. It is **not** the singular record: one published spec is a **ToolCatalogRecord** (CLI: a *catalog record*), never "a catalog". The v1 `ManufacturerCatalog` table/router (deep-model substrate) is retired — the R6 evict-the-squatter move, same as "preset". |
...
| **Coverage** | a set-vs-machine diff view (+ `absent_on_machine`/`machine_only`/`number_mismatch`) | the **setup view** (`loobric status`): claims + notes, derived, advisory-only (re-earned 2026-07-29, MAPPING_PLAN) |
| **Reconcile** | aligning set numbering to machine tool-table entries | user/agent-facing: **status** / the setup view (the old sense — *writing* machine numbers over set numbers — is gone for good: claims are durable). The word survives only in server-internal code (`reconcile_set_membership`, `reconcile_machine`); public paths and bundled clients use **status** (CI-enforced). |
| **Needs Attention** | a FreeCAD tab duplicating the Inbox | that tab stays dead; the *phrase* "need attention" was re-earned 2026-07-29 as the setup view's severity class ("2 need attention, 3 notes") — human surfaces only, still denied on the agent/MCP surface |
| **mirror / mirrors / link / linked** | the ToolSet↔Machine relation | a **Setup** (`use-set` / `setup-history`) — "link" retired for this relation 2026-07-29 with the set-level `machine_id` itself; "bind/bound" remains entry↔instance only |
| **loaded** (member state) | a machine-linked set member whose instance was bound (its number then *overwritten* by observation) | **satisfied** — same fact, but the claim survives: observation rides alongside (`observed`), never over it (MAPPING_PLAN §5.1, 2026-07-29) |
...
| **Tool number** | The ONLY tool identifier that travels in G-code (`T3 M6`): the single point of contact between CAM's assumption and the controller's reality. Both sides' number→tool mappings are recorded (CAM's durable **claim** on the ToolSet member; the machine side via Binding on `(machine, tool_number)`), making their agreement a verifiable fact — the setup view (`loobric status`) IS that verification. Verifying this mapping is the system's most important job — see `CONCEPTS.md` and `MAPPING_PLAN.md`. |
...
| **API key** | User-created credential for programmatic/machine access (what a controller-side sync script uses). Distinct from email/password login. Carries **scopes** (below); creating one requires choosing them. |
| **Scope (API key)** | A door an API key may use: `read`, `sync`, `observe`, `assert`, `bind`, `delete`, `admin` — the scopes ARE the canonical doors (0.6.0, SCOPES_PLAN.md). Enforced per request for key auth; sessions (a signed-in human) and solo mode are unscoped. One or-mapping exists: machine self-registration (`POST /machine-records` + its `/assert`) accepts `observe` OR `assert`, so a controller key survives first contact (SCOPES_PLAN amendment 2026-07-30). The canonical AI-agent key is `read sync assert` — an agent's key *cannot* observe, bind, or delete, making "agents assert, never observe" a credential property, not a convention. Keys created before 0.6.0 ("legacy") degrade to read-only until replaced. |
| **Tag-based API access** | Scoping an API key's reach by entity tags (e.g. a key for machine `mill01` only sees tools tagged for it). ⚠️ Pre-reboot concept, never enforced by the v2 surface; retained in storage only. Not part of the 0.6.0 scope model. |
